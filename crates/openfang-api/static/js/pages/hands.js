  ...
  return {
    ...
    async loadData() {
      ...
      try {
        ...
        this.hands = [];
        this.loadError = e.message || '无法加载手部工具。';
      }
      this.loading = false;
    },
    ...
    async openSetupWizard(handId) {
      ...
      try {
        ...
        this.showToast('无法加载手部工具详情：' + (e.message || '未知错误'));
      }
    ...
    async installDeps() {
      ...
      try {
        ...
        this.installProgress.status = '完成';
        ...
        if (data.requirements_met) {
          ...
          this.installProgress.error = failed + ' 个依赖安装失败，请查看下方详情。';
        }
      } catch(e) {
        this.installProgress = {
          status: 'error',
    ...
    async recheckDeps() {
      ...
      try {
        ...
        if (data.requirements_met) {
          this.showToast('所有依赖已满足！');
        }
      } catch(e) {
        this.showToast('检查失败：' + (e.message || '未知'));
      }
    ...
    async launchHand() {
      ...
      try {
        ...
        this.showToast('手部工具 "' + handId + '" 已激活为 ' + (data.agent_name || data.instance_id));
    ...
    async deactivate(inst) {
      ...
      OpenFangToast.confirm('停用手部工具', '停用手部工具 "' + handName + '"？这将终止其代理。', async function() {
        try {
    ...
    async loadStats(inst) {
      try {
        ...
        inst._stats = { '错误': { value: e.message || '无法加载统计数据', format: 'text' } };
      }
    },
    ...
    async refreshBrowserView() {
      ...
      try {
        ...
        if (data.active) {
          ...
          this.browserViewer.error = '无活跃的浏览器会话';
