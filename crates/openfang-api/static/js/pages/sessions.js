...
function sessionsPage() {
  return {
    ...
    deleteSession(sessionId) {
      var self = this;
      OpenFangToast.confirm('删除会话', '这将永久删除该会话及其消息。', async function() {
        try {
    ...
    async addKey() {
      ...
      try {
        ...
        OpenFangToast.error('保存键失败：' + e.message);
      }
    },

    deleteKey(key) {
      var self = this;
      OpenFangToast.confirm('删除键', '确定删除键 "' + key + '" 吗？此操作无法撤销。', async function() {
        try {
