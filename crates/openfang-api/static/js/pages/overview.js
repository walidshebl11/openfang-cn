...
function overviewPage() {
  return {
    ...
    get configuredProviders() {
      return this.providers.filter(function(p) { return p.auth_status === '已配置'; });
    },
    ...
    providerTooltip(p) {
      ...
      if (p.health === 'open') return p.display_name + ' \u2014 熔断器开启';
      if (p.auth_status === '已配置') return p.display_name + ' \u2014 就绪';
    ...
    get setupChecklist() {
      return [
        { key: 'provider', label: '配置 LLM 提供商', done: this.configuredProviders.length > 0, action: '#settings' },
        { key: 'agent', label: '创建你的第一个智能体', done: (Alpine.store('app').agents || []).length > 0, action: '#agents' },
        { key: 'chat', label: '发送你的第一条消息', done: localStorage.getItem('of-first-msg') === 'true', action: '#chat' },
        { key: 'channel', label: '连接消息通道', done: this.channels.length > 0, action: '#channels' },
