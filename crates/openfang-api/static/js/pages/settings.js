...
function settingsPage() {
  return {
    ...
    coreFeatures: [
      ...
      {
        name: 'SSRF 防护', key: 'ssrf_protection',
    ...
    async loadSettings() {
      ...
      try {
        ...
        this.loadError = e.message || '无法加载设置。';
      }
      this.loading = false;
    },
    ...
    providerAuthClass(p) {
      if (p.auth_status === '已配置') return 'auth-configured';
      if (p.auth_status === 'not_set' || p.auth_status === 'missing') return 'auth-not-set';
      return 'auth-no-key';
    },

    providerAuthText(p) {
      if (p.auth_status === '已配置') return '已配置';
      if (p.auth_status === 'not_set' || p.auth_status === 'missing') return '未设置';
      return '无需密钥';
    },
      ...
      setTimeout(async function() {
        ...
        try {
          ...
          if (resp.status === 'complete') {
            OpenFangToast.success('GitHub Copilot 认证成功！');
            ...
            OpenFangToast.error('设备码已过期，请重试。');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else if (resp.status === 'denied') {
            OpenFangToast.error('用户拒绝访问。');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else {
    ...
    async saveProviderUrl(provider) {
      ...
      if (!url || !url.trim()) { OpenFangToast.error('请输入基础 URL'); return; }
      url = url.trim();
      ...
      try {
        ...
        if (result.reachable) {
          OpenFangToast.success(provider.display_name + ' URL 已保存 — 可访问（' + (result.latency_ms || '?') + 'ms）');
        } else {
          OpenFangToast.warning(provider.display_name + ' URL 已保存但无法访问');
        }
        await this.loadProviders();
      } catch(e) {
    ...
    formatConfigValue(feature) {
      ...
      switch (feature.valueKey) {
        case 'rate_limiter':
          return '算法：' + (val.algorithm || 'GCRA') + ' | ' + (val.tokens_per_minute || 500) + ' 令牌/分钟/IP';
        case 'websocket_limits':
          return '最多 ' + (val.max_per_ip || 5) + ' 连接/IP | ' + Math.round((val.idle_timeout_secs || 1800) / 60) + '分钟空闲超时 | ' + Math.round((val.max_message_size || 65536) / 1024) + 'KB 最大消息';
        case 'wasm_sandbox':
          return '燃料：' + (val.fuel_metering ? '开启' : '关闭') + ' | 时间片：' + (val.epoch_interruption ? '开启' : '关闭') + ' | 超时：' + (val.default_timeout_secs || 30) + '秒';
        case 'auth':
          return '模式：' + (val.mode || '未知') + (val.api_key_set ? '（已配置密钥）' : '（未设置密钥）');
        default:
    ...
    formatMonitoringValue(feature) {
      ...
      switch (feature.valueKey) {
        case 'audit_trail':
          return (val.enabled ? '活跃' : '已禁用') + ' | ' + (val.algorithm || 'SHA-256') + ' | ' + (val.entry_count || 0) + ' 条日志';
        case 'taint_tracking':
          var labels = val.tracked_labels || [];
          return (val.enabled ? '活跃' : '已禁用') + ' | 跟踪：' + labels.join(', ');
    ...
    async loadPeers() {
      ...
      try {
        ...
        this.peers = [];
        this.peersLoadError = e.message || '无法加载节点。';
      }
