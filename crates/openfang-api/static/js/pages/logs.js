...
function logsPage() {
  return {
    ...
    friendlyAction: function(action) {
      if (!action) return '未知';
      var map = {
        'AgentSpawn': 'Agent 已创建', 'AgentKill': 'Agent 已停止', 'AgentTerminated': 'Agent 已停止',
        'ToolInvoke': '工具已使用', 'ToolResult': '工具已完成', 'AgentMessage': '消息',
        'NetworkAccess': '网络访问', 'ShellExec': 'Shell 命令', 'FileAccess': '文件访问',
        'MemoryAccess': '内存访问', 'AuthAttempt': '登录尝试', 'AuthSuccess': '登录成功',
        'AuthFailure': '登录失败', 'CapabilityDenied': '权限被拒绝', 'RateLimited': '频率限制'
      };
    ...
    async verifyChain() {
      try {
        ...
        if (this.chainValid) {
          OpenFangToast.success('审计链验证通过 — ' + (data.entries || 0) + ' 条记录有效');
        } else {
          OpenFangToast.error('审计链已损坏！');
        }
      } catch(e) {
        this.chainValid = false;
        OpenFangToast.error('链验证失败：' + e.message);
      }
    },
